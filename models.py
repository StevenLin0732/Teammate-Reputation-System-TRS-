...
class User(db.Model):
    ...
    def reputation(self):
        ...
        would_work_again_count = db.session.query(func.sum(func.case([(Rating.would_work_again == True, 1)], else_=0))).filter(Rating.target_user_id == self.id).scalar() or 0
...
class Submission(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'team_id': self.team_id, 'proof_link': self.proof_link}
